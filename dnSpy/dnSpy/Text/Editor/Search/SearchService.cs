...
#pragma warning restore CS0169
			...
			if (inIncrementalSearch) {
				if (group == CommandConstants.TextEditorGroup) {
					...
					case TextEditorIds.TYPECHAR:
						...
						if (s is not null && s.IndexOfAny(LineConstants.newLineChars) < 0)
		...
		void ShowSearchControl(SearchKind searchKind, bool canOverwriteSearchString) {
			...
			if (searchControl is null) {
				...
				searchControl.InputBindings.Add(new KeyBinding(new RelayCommand(a => MatchWholeWords = !MatchWholeWords), new AltGrKeyGesture(Key.W)));
				searchControl.InputBindings.Add(new KeyBinding(new RelayCommand(a => UseRegularExpressions = !UseRegularExpressions), new AltGrKeyGesture(Key.E)));
		...
		void ReplaceNext() {
			...
			if (res is null)
				return;
			Debug2.Assert(expandedReplacePattern is not null);
		...
		void UpdateTextMarkerSearch() {
			...
			if (SearchString.Length == 0) {
				...
				if (oldColl is not null && oldColl.Count != 0)
					RefreshAllTags();
				SetFoundResult(true);
				return;
			}
